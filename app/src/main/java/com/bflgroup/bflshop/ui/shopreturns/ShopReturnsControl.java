...
//        }
    ...
    ArrayList<AddScanItemDetails> gettmpitem() throws SQLException {
        ...
        String querynew = "select * from tmpshopreturns where  devicename = '" + objGlobal.getDeviceName() + "'";
    ...
    public boolean InsertAddDetails(String remarks, String Category, String Shopname, Context context) throws SQLException {
        ...
        try {
            String query = "select * from tmpshopreturns where devicename = '" + objGlobal.getDeviceName() + "'";
            String query2 = "select sum(qty) as qty, salesprice as salesprice,itemcode from tmpshopreturns where devicename = '" + objGlobal.getDeviceName() + "' group by itemcode,salesprice";
            String query6 = "select description, qty, ShortName, UnitCode,salesprice, itemcode, GroupCode,CatCode,OpeningDate,ToPrint from tmpshopreturns where devicename = '" + objGlobal.getDeviceName() + "' group by itemcode,salesprice, qty, description, ShortName,UnitCode, GroupCode,CatCode,OpeningDate,ToPrint";
...
//            }
    ...
    public ArrayList<String> loadCategory() {
        ...
        try {
            ...
            String Query = "select Category_name from bfldata..StoreEntryCategory where active ='Y' union all select Category_name from " +
                    "bfldata.dbo.StoreEntryCategoryShopTransfer where ShopName='" + objPosGlobal.getShopName() + "' order by Category_Name";
            ResultSet rs1 = dbConnection.getResultSet(Query, objGlobal.getCloudCon());
            while (rs1.next()) {
