...
public class AgeingSlashingWifyControl {
    ...
    public boolean getAgeingItems(String batchno, String itemcode,float scanprice) {
        ...
        try {
            ...
            rs = dbConnection.getResultSet("select AddInfo = ISNULL(AddInfo, '') +'-'+ FORMAT(GETDATE(), 'MMyy'),ASubclass,Abrand from itemmh where itemcode='" + itemcode + "'", objGlobal.getConnection());
